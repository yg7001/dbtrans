;; Modified  by Gu Zhigang (yg7001) 2018-09-25
...
(defprotocol ControlableDataSource
  (get-conn[this])
)
(defrecord DataSource [datasource]
  java.io.Closeable
  ControlableDataSource
  (close [_]
    (.close datasource)
  )
  (get-conn [this] 
...
(defn make-datasource-spec
  ...
  (let [dbspec (normalize-dbspec dbspec)
    ...
    ;(log/debug "***********  dbspec to make is : " dbspec)
    (if (:connection-uri dbspec)
      (.setJdbcUrl ds (:connection-uri dbspec))
      (let [suri (str "jdbc:"  (:subprotocol dbspec) ":"  (:subname dbspec))]
        ;(log/debug "***************[ JdbcUrl ]=" suri)
        (.setJdbcUrl ds suri)))
    (->DataSource (doto ds
